...
\(F=\{0\}\cup\{1/m:m\in\Npos\}\)，可依次连接前 \(k\) 个非零点并插入 \(0\)，使每个点出现无穷次，同时所有非零点只在 \(0\) 附近累积。一般可数闭集若枚举为 \(\{x_j\}\)，连接有限块
...
\(\abs{a_{n_j}-L}<2/j\)，所以 \(L\) 也是部分极限，集合闭。最大、最小元由\exerciseref{12.9}、\exerciseref{12.10}分别识别为上下极限。
\end{solution}
