...
\(v_n=\sum_{j<d}b_jn^{j-d}\)。令
\[
 A=\sum_{j<d}\abs{a_j},\qquad B=\sum_{j<d}\abs{b_j}.
\]
由于 \(j-d\le-1\)，对 \(n\ge1\) 有
\(\abs{u_n}\le A/n\)、\(\abs{v_n}\le B/n\)。若
\(n\ge 2B/\abs{b_d}\)，则
...
因此
\[
 \abs{\frac{p(n)}{q(n)}-\frac{a_d}{b_d}}
 \le
 \frac{2(\abs{b_d}A+\abs{a_d}B)}{\abs{b_d}^2}\frac1n
 =:\frac Cn.
\]
给定 \(\varepsilon>0\)，取整数
\(
 N>\max\{2B/\abs{b_d},C/\varepsilon\}
\)
即可。
\end{solution}
...
\begin{solution}{8.6}
取 \(m\in\Npos\) 使 \(1/q>1+1/m\)。由 Bernoulli 不等式，
\[
...
给定 \(\varepsilon>0\)，令
\[
 \delta=\min\left\{1,\frac{\varepsilon}{1+\abs a+\abs b}\right\}.
\]
取共同尾部使
\(\abs{a_n-a}<\delta\)、\(\abs{b_n-b}<\delta\)。由题给分解，
\[
\begin{aligned}
 \abs{a_nb_n-ab}
 &\le \abs{a_n-a}\abs{b_n-b}
   +\abs a\abs{b_n-b}
   +\abs b\abs{a_n-a}\\
 &<\delta^2+(\abs a+\abs b)\delta\\
 &\le(1+\abs a+\abs b)\delta\le\varepsilon.
\end{aligned}
\]
若最后可能取等号，可在定义 \(\delta\) 时把 \(\varepsilon\) 换成 \(\varepsilon/2\)，从而得到严格小于 \(\varepsilon\) 的估计。
\end{solution}
