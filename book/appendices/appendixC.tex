...
\[
 f=O(g),\qquad f=o(g)
\]
分别表示：在指定极限过程中，\(g\) 最终非零且 \(|f/g|\) 有界、\(f/g\to0\)。若参数还在变化，应说明界与收敛对哪些参数一致。

吸收法把
\[
 X\le A+\theta X,\qquad 0\le\theta<1
\]
改写为 \(X\le A/(1-\theta)\)。当 \(\theta\) 依赖参数时，必须在所研究的参数范围内给出统一的 \(\theta\le\theta_0<1\)。
...
设 \(u:[0,T]\to[0,\infty)\) 连续，\(b:[0,T]\to[0,\infty)\) Lebesgue 可积，并且对每个 \(t\in[0,T]\) 都有
\[
...
\begin{proof}
连续函数 \(u\) 在紧区间上有界，故 \(bu\in L^1[0,T]\)。若 \(A>0\)，令 \(v(t)=A+\int_0^tb(s)u(s)\,ds\)。则 \(v\) 绝对连续、\(u\le v\)，且几乎处处
\[
...
从 \(0\) 到 \(t\) 积分得到
\[
 \log v(t)-\log A\le\int_0^t b(s)\,ds,
\]
所以 \(u(t)\le v(t)\le A\exp(\int_0^t b)\)。当 \(A=0\) 时，原不等式也蕴含
\[
 u(t)\le \varepsilon+\int_0^t b(s)u(s)\,ds
\]
对每个 \(\varepsilon>0\) 成立。应用已经证明的情形后令 \(\varepsilon\downarrow0\)，得到 \(u(t)=0\)。
\end{proof}

还可采用更弱的假设：\(u,b,bu\in L^1\)，积分不等式几乎处处成立。此时可重新选择 \(u\) 的代表，使结论在相应的几乎处处意义下成立。正文使用 Grönwall 不等式时会明确采用哪个版本。
...
设 \(K:\R^d\to\C\) 可积，并对 \(\varepsilon>0\) 定义
\[
 K_\varepsilon(x)=\varepsilon^{-d}K(x/\varepsilon).
\]
变量代换给出
\[
 \|K_\varepsilon\|_{L^1}=\|K\|_{L^1},\qquad
 \|K_\varepsilon\|_{L^p}
 =\varepsilon^{-d(1-1/p)}\|K\|_{L^p}.
\]
在一阶导数存在且可积时还有
\[
 \|\nabla K_\varepsilon\|_{L^1}
 =\varepsilon^{-1}\|\nabla K\|_{L^1}.
\]
这些幂次来自体积元的 \(\varepsilon^d\) 与幅度归一化的
\(\varepsilon^{-d}\)，应由变量代换核对。

若 \(\int_{\R^d}K=1\)，并且
\[
 \int_{|x|>R}|K(x)|\,dx\longrightarrow0
 \qquad(R\to\infty),
\]
则 \(K_\varepsilon\) 的质量随着 \(\varepsilon\downarrow0\) 集中到原点。对有界一致连续函数 \(f\)，估计
\[
 |K_\varepsilon*f(x)-f(x)|
 \le \int |K_\varepsilon(y)|\,|f(x-y)-f(x)|\,dy
\]
可按 \(|y|<\delta\) 与 \(|y|\ge\delta\) 分开：前者由一致连续性控制，后者由核的尾质量控制。这是近似恒等族收敛证明的基本分解。

卷积还满足 Young 的端点估计。若 \(K\in L^1(\R^d)\)、\(f\in L^\infty(\R^d)\)，则
\[
 \|K*f\|_{L^\infty}
 \le\|K\|_{L^1}\|f\|_{L^\infty}.
\]
若 \(f\in L^p\)、\(1\le p<\infty\)，Minkowski 积分不等式给出
\[
 \|K*f\|_{L^p}\le\|K\|_{L^1}\|f\|_{L^p}.
\]
卷积、Lebesgue 积分和 \(L^p\) 空间在后卷正式建立；本节只登记以后反复使用的尺度与常数。
