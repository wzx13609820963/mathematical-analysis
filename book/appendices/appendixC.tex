...
\noindent\textbf{配置说明：}本附录按III级工具附录配置，共12道编号题，含21个训练单元。

\begin{enumerate}[label=\textbf{C.\arabic*.}]
 \exerciseitem{C.1} \mustdo 证明有限和三角不等式与范数的反三角不等式。
 \exerciseitem{C.2} \mustdo 用二次多项式非负证明 Cauchy--Schwarz 不等式及等号条件。
 \exerciseitem{C.3} \mustdo 从 Young 不等式推导有限序列 Hölder 不等式。
 \exerciseitem{C.4} \mustdo 补全 Minkowski 不等式证明中指数 \((p-1)q=p\) 的计算与零范数情形。
 \exerciseitem{C.5} \mustdo 证明有限 Jensen 不等式，并分析严格凸等号条件。
 \exerciseitem{C.6} \mustdo 证明离散 Grönwall 不等式；再处理 \(u_n\le A_n+\sum_{k<n}b_ku_k\) 且 \(A_n\) 单调的版本。
 \exerciseitem{C.7} \mustdo 严格处理连续 Grönwall 中 \(A=0\) 的极限步骤。
 \exerciseitem{C.8} \mustdo 证明截断映射 \(T_M\) 是 \(1\)-Lipschitz。
 \exerciseitem{C.9} \mustdo 为三项误差分别分配 \(\varepsilon/2,\varepsilon/4,\varepsilon/4\)，说明与 \(\varepsilon/3\) 的等价作用。
 \exerciseitem{C.10} \mustdo 证明 Cesàro 平均定理，并构造逆命题反例。
 \exerciseitem{C.11} \projectdo\ 【前置：单调性与求和分部；预计用时：60分钟】证明 Stolz--Cesàro 定理的有限极限及 \(+\infty\) 版本。
 \exerciseitem{C.12} \optionaldo 对调和数 \(H_n\) 应用 Stolz--Cesàro，证明 \(H_n/\log n\to1\)；可使用 \(\log(1+1/n)\sim1/n\)。
\end{enumerate}
