...
\begin{description}
 \item[于品，《数学分析之课程讲义》.]
 这份清华大学课程讲义较早引入距离空间和赋范空间，并把若干较长的经典结果拆成连续习题。它适合观察一门分析课程怎样在基本极限理论之外安排几何、积分、分布与应用材料。阅读时须注意其定位是随课堂推进的讲义，版本说明也明确提醒读者核对笔误。
 \item[Amann--Escher, \emph{Analysis I}.]
 第一卷依次处理基础、收敛、连续函数、微分和函数序列，证明展开较充分，并用大量例子和习题维持章节之间的连续性。它适合与本书第一卷逐章对照：比较同一定义需要哪些前置结果，以及一个存在性定理在后续承担哪些证明职责。
 \item[Godement, \emph{Analysis I}.]
 该书把集合与函数、离散收敛、连续变量的收敛以及初等函数放在同一卷中，叙述不刻意压缩。其价值主要在于展示概念、例子和证明之间可以保留较长的解释距离；不宜只按目录标题逐项比照。
...
\begin{description}
 \item[严格基础路线] \chapterref{chap:mathematical-language} $\to$ \chapterref{chap:real-number-system} $\to$ \chapterref{chap:dedekind-construction}或\chapterref{chap:cauchy-construction} $\to$ \chapterref{chap:equivalent-completeness} $\to$ 第二、三编。实数构造中较技术性的证明可以在完成数列极限后回读。
 \item[极限专题路线] \chapterref{chap:mathematical-language}量词与证明方法 $\to$ \chapterref{chap:real-number-system}确界 $\to$ \chapterref{chap:equivalent-completeness}完备性 $\to$ \chapterref{chap:sequence-limits}至\chapterref{chap:cauchy-criterion} $\to$ \chapterref{chap:function-limits}。该路线突出估计、子列与 Cauchy 准则。
 \item[连续性专题路线] \chapterref{chap:real-number-system}确界与稠密性 $\to$ \chapterref{chap:sequence-limits}及\chapterref{chap:subsequences-cluster-points} $\to$ \chapterref{chap:function-limits} $\to$ \chapterref{chap:continuity-discontinuity}至\chapterref{chap:uniform-continuity}。上、下极限一章可作为并行补充，不是连续性主线的直接前置。
 \item[结构比较路线] \chapterref{chap:dedekind-construction}至\chapterref{chap:equivalent-completeness} $\to$ \chapterref{chap:cauchy-criterion} $\to$ \chapterref{chap:continuous-functions-closed-interval}。重点比较有序域、完备度量思想、紧致和连通。
\end{description}
...
\begin{description}
 \item[自然数] 有的作者取 $\N=\{1,2,\ldots\}$，有的取 $\N=\{0,1,2,\ldots\}$。本书固定采用后一约定，并写 $\Npos=\{1,2,3,\ldots\}$；涉及分母或递归起点时不省略正整数限制。
...
本附录导读所据版本如下：
\begin{enumerate}
 \item 于品，《数学分析之课程讲义》（丘成桐数学英才班试用），清华大学数学系及丘成桐数学科学中心，2020年12月讲义版。
 \item Herbert Amann and Joachim Escher, \emph{Analysis I}, Birkh\"auser, 2005.\\
 DOI: \nolinkurl{10.1007/b137107}.
 \item Roger Godement, \emph{Analysis I: Convergence, Elementary Functions}, Springer, 2004.\\
 DOI: \nolinkurl{10.1007/978-3-642-18491-8}.
...
\begin{enumerate}[label=\textbf{G.\arabic*.}]
 \exerciseitem{G.1} 从\chapterref{chap:sequence-limits}或\chapterref{chap:function-limits}任选一个极限定理，把其量词结构完整写出，并说明 $\varepsilon$--$N$ 与 $\varepsilon$--$\delta$ 语言各自控制的变量。
 \exerciseitem{G.2} 说明为何“定理以某人命名”不能单独证明历史优先权，并列出核查一项优先权主张所需的两类材料。
 \exerciseitem{G.3} 查表解释英语 \emph{sequence}、\emph{series}、\emph{subsequence} 的区别，并给出相应德语术语。
 \exerciseitem{G.4} 比较“紧致”“序列紧致”“闭且有界”三个表述在 $\R$、度量空间和一般拓扑空间中的关系。
 \exerciseitem{G.5} 为一本数学专著写出完整书目条目，并说明 DOI、版本号和访问日期各自解决什么识别问题。
 \projectexerciseitem{G.6} \ 【前置：前三编；预计用时：60分钟】选择“实数完备性”或“连续函数”专题，按本附录的阅读路线列出一页学习地图：每个节点写明定义、一个主要定理、一个反例以及对下一节点的依赖。涉及后续正文的节点只标注预留位置。
\end{enumerate}
