...
\]
则称 \(I\) 为理想，记作 \(I\lhd R\)。理想是构造环商所需的封闭条件。
\end{definition}
...
\begin{enumerate}[label=\textbf{F.\arabic*.}]
 \exerciseitem{F.1} \mustdo 证明群的单位元和每个元素的逆元唯一。
 \exerciseitem{F.2} \mustdo 证明群同态保持单位元和逆元，并证明单射等价于核平凡。
 \exerciseitem{F.3} \mustdo 验证环同态的核是理想、像是子环。
 \exerciseitem{F.4} \mustdo 证明正规性正是陪集乘法良定义所需的条件。
 \exerciseitem{F.5} \mustdo 完整证明群的第一同构定理。
 \exerciseitem{F.6} \mustdo 对求值同态 \(F[x]\to F\) 证明核为 \((x-a)\)，并写出对应商同构。
 \exerciseitem{F.7} \mustdo 证明复共轭、模乘法和复三角不等式。
 \exerciseitem{F.8} \mustdo 求 \(z^n=1\) 的全部根，并证明它们组成乘法循环群。
 \exerciseitem{F.9} \mustdo 证明实线性映射的复化延拓存在且唯一。
 \exerciseitem{F.10} \mustdo 证明带余除法和因式定理。
 \exerciseitem{F.11} \mustdo 证明域上非零 \(n\) 次多项式至多有 \(n\) 个不同根。
 \exerciseitem{F.12} \mustdo 假设代数基本定理，证明实多项式可分解为实一次因子与不可约实二次因子。
 \exerciseitem{F.13} \mustdo 验证形式幂级数 Cauchy 乘积结合律的每个系数只涉及有限和。
 \exerciseitem{F.14} \mustdo 证明形式幂级数可逆当且仅当常数项非零，并计算 \(1/(1-x)\) 的形式逆。
 \exerciseitem{F.15} \mustdo 说明形式恒等式 \((1-x)\sum_{n\ge0}x^n=1\) 与数值级数收敛是两个不同命题。
 \exerciseitem{F.16} \mustdo 用张量积泛性质说明双线性型等价于 \(V\otimes V\) 上的线性泛函。
 \exerciseitem{F.17} \mustdo 证明外积反对称，并计算 \(\dim\Lambda^k(F^n)\)。
 \exerciseitem{F.18} \projectdo\ 【前置：附录B与本附录；预计用时：90分钟】用最高次外幂重建行列式的乘法性、换基公式和定向体积解释。
\end{enumerate}
