...
\begin{description}[style=nextline,leftmargin=2.8em]
 ...
 \item[Cauchy 但值域中无极限]
 \chapterref{chap:rational-defects}逼近 \(\sqrt2\) 的有理数列在 \(\Q\) 中是 Cauchy 列，却没有有理极限。失败条件是值域不完备；同一列在 \(\R\) 中收敛。
 \item[无界但有收敛子列]
...
\begin{enumerate}[label=\textbf{D.\arabic*.}]
 \exerciseitem{D.1} \mustdo 为“有界但不收敛”给出两个机制不同的数列，并分别指出其收敛子列。
 \exerciseitem{D.2} \mustdo 构造相邻差趋零但非 Cauchy 的数列，并用双指标差证明。
 \exerciseitem{D.3} \mustdo 解释有理 Cauchy 列无有理极限时，失败的是哪一条假设而不是 Cauchy 定义。
 \exerciseitem{D.4} \mustdo 构造无界但有两个不同有限部分极限的数列。
 \exerciseitem{D.5} \mustdo 为连续但不一致连续给出“逃向无穷远”和“逼近缺失边界”两类反例点对。
 \exerciseitem{D.6} \mustdo 证明 \(\sqrt x\) 一致连续但非 Lipschitz。
 \exerciseitem{D.7} \mustdo 给出严格单调但不连续的函数，并求其像集。
 \exerciseitem{D.8} \mustdo 为 \(\infty-\infty\) 与 \(0\cdot\infty\) 各构造至少三种结果。
 \exerciseitem{D.9} \mustdo 分别说明 \((0,1)\) 与 \(\R\) 的非紧致机制。
 \exerciseitem{D.10} \mustdo 证明 \(\Q\) 在通常距离下不完备，并指出它在 \(\R\) 中不闭。
 \exerciseitem{D.11} \projectdo\ 【前置：前三编；预计用时：75分钟】选取一个主定理，制作“删条件—反例—失败步骤—可恢复结论”四列表。
 \exerciseitem{D.12} \opendo 说明为什么预留条目在依赖章节完成前不宜写成正式反例索引；至少讨论符号、定理版本和逻辑循环三类风险。
\end{enumerate}
