...
以下先在一个有序域 \(F\) 中表述各项命题；应用于实数时取 \(F=\R\)。本章必须在\chapterref{chap:sequence-limits}系统研究数列极限之前使用少量序列语言，现统一规定如下。数列是映射 \(a:\Npos\to F\)，记为 \((a_n)\)。称 \(a_n\) 收敛到 \(a\in F\)，如果
\[
 \forall\varepsilon>0\ \exists N\in\Npos\ \forall n\ge N,
 \qquad |a_n-a|<\varepsilon.
\]
严格递增的指标列 \(n_1<n_2<\cdots\) 确定子列 \((a_{n_k})\)。称 \((a_n)\) 为 Cauchy 列，如果
\[
 \forall\varepsilon>0\ \exists N\in\Npos\ \forall m,n\ge N,
 \qquad |a_m-a_n|<\varepsilon.
\]
本章只比较这些命题与实数完备性的关系；量词否定、运算和估计方法留到\chapterref{chap:sequence-limits}展开。

\section{确界原理与单调有界收敛}

\subsection{各种完备性命题及其依赖图}


考虑以下命题：
\begin{description}[style=nextline,leftmargin=3em]
 \item[(S) 确界原理] 每个非空有上界集合 \(A\subseteq F\) 有上确界。
 \item[(M) 单调有界收敛] 每个单调递增且有上界的 \(F\)-值数列收敛；递减情形对称。
...
另记
\begin{description}[style=nextline,leftmargin=3em]
 \item[(A) Archimedes 性质] 对每个 \(x\in F\)，存在自然数 \(n>x\)。
\end{description}

这些命题回答的是同一个缺口的不同表现，但量化对象并不相同。(S) 对任意非空有界集合作断言，(M)、(B)、(C) 只考察数列，(N) 考察一列闭区间。把它们列在一起，是为了说明在实数的序和 Archimedes 尺度下，集合边界可以编码成数列，数列又可以装入逐渐缩小的区间。

证明按下列次序进行：确界原理先由\chapterref{chap:real-number-system}的定理给出 (A)，随后
\[
 \mathrm{(S)}\Longrightarrow\mathrm{(A)},\qquad
...
\]
二分区间长度趋零以及最后一个方向都使用 (A)。把这一条件单独列出，是为了避免把“有序域中的形式推理”和“自然数尺度能够任意细分”混为一谈。
...
\begin{proposition}[有理数中的失败]
\(\Q\) 满足有序域公理和 Archimedes 性质，却不满足 (S)、(M)、(N)、(B)、(C) 的实数版本。因而 Archimedes 性质只负责尺度趋零，不能单独提供缺失极限。
\end{proposition}

\begin{proof}
\chapterref{chap:real-number-system}的有理 Cauchy 列直接否定 (C)。其单调有界十进制逼近列没有有理极限，否定 (M)；相应左右有理端点构成长度趋零而交集在 \(\Q\) 中为空的闭区间套，否定 (N)。无有理上确界的集合否定 (S)。若 (B) 在 \(\Q\) 中成立，该有理逼近列会有有理收敛子列，但平方误差迫使子列极限平方为 \(2\)，矛盾。
\end{proof}

\subsection{确界原理与单调有界收敛定理}


\begin{theorem}[单调有界收敛]
若 \((a_n)\) 单调递增且有上界，则
\[
a_n\to\sup\{a_n:n\in\Npos\}.
\]
若单调递减且有下界，则它收敛到项集的下确界。
\end{theorem}

\begin{proof}
令 \(s=\sup\{a_n:n\in\Npos\}\)。给定 \(\varepsilon>0\)，\(s-\varepsilon\) 不是上界，所以存在 \(N\) 使 \(a_N>s-\varepsilon\)。单调性给出
\[
...
\begin{proof}
从 \(E\) 中递归选取互异点 \(x_n\)。这一递归使用可数选择的一种形式；本书采用 ZFC，故该步骤合法，选择原则的区分见附录A。所得有界数列有收敛子列 \(x_{n_k}\to x\)。因子列各项互异，对每个 \(\varepsilon>0\) 都有充分大的 \(k\) 使 \(x_{n_k}\in(x-\varepsilon,x+\varepsilon)\)，且至多一项等于 \(x\)。故每个邻域都含异于 \(x\) 的 \(E\) 中点，\(x\) 是聚点。
\end{proof}
...
章首已经给出 Cauchy 条件。与收敛不同，它不含候选极限，只要求同一尾部内任意两项彼此接近。
...
\]
若尾部差集无上界，局部约定其上确界为符号 \(+\infty\)；这里只把它作为“无有限上界”的记号，不使用\chapterref{chap:limsup-liminf}扩充实数的运算。则 \((a_n)\) 是 Cauchy 列当且仅当 \(d_N\) 最终有限且
\[
 d_N\to0.
\]
\end{proposition}
...
\begin{proof}
设 \(\varnothing\ne A\subseteq F\) 有上界。先取 \(a_0\in A\)，令 \(l_0=a_0-1\)，则 \(l_0\) 不是上界；再取一个上界 \(u_0\)，必要时放大它使 \(l_0<u_0\)。已给 \(l_n,u_n\) 后令 \(m_n=(l_n+u_n)/2\)：若 \(m_n\) 是上界，取
\[
...
\[
 u_n-l_n=2^{-n}(u_0-l_0).
\]
Archimedes 性质使右端趋于零：给定 \(\varepsilon>0\)，先取自然数 \(N>(u_0-l_0)/\varepsilon\)，再用 \(2^N\ge N\) 即得 \(2^{-N}(u_0-l_0)<\varepsilon\)。若 \(m\ge n\)，嵌套性给出
\[
  l_n\le l_m\le u_m\le u_n,
\]
因而
\[
 |l_m-l_n|\le u_n-l_n,\qquad
 |u_m-u_n|\le u_n-l_n.
\]
所以两列都是 Cauchy 列。设 \(l_n\to l\)、\(u_n\to u\)。给定 \(\varepsilon>0\)，充分大时同时有
\[
 |u-u_n|<\frac{\varepsilon}{3},\qquad
 u_n-l_n<\frac{\varepsilon}{3},\qquad
 |l_n-l|<\frac{\varepsilon}{3}.
\]
三角不等式给出 \(|u-l|<\varepsilon\)。因 \(\varepsilon\) 任意，若 \(u\ne l\)，取 \(\varepsilon=|u-l|/2\) 即得矛盾，故 \(l=u=:s\)。
...
\subsection{Archimedes 条件在等价证明中的作用}
...
\begin{enumerate}[label=\textbf{\thechapter.\arabic*.},resume]
 ...
 \optionalexerciseitem{6.15} 证明闭区间的可数开覆盖若具有 Lebesgue 数，则可抽取有限子覆盖；说明区间套思想的作用。
 \openexerciseitem{6.16} 调查一个非 Archimedes 有序域，说明 Cauchy 完备、序完备与球完备三种概念的区别。
 ...
 \openexerciseitem{6.26} 比较可数区间套与任意全序嵌套闭区间族的交性质，说明为何后者会导向比序列完备更强的完备概念。
\end{enumerate}
