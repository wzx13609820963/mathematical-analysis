...
类 \(x=[a_n]\) 可由其任一足够靠后的有理项逼近。给定 \(\varepsilon>0\)，先对 \(\varepsilon/2\) 使用 Cauchy 条件，取 \(N\) 使 \(n\ge N\) 时
\[
 |a_n-a_N|<\frac{\varepsilon}{2}.
\]
于是类 \(\varepsilon-|a_n-a_N|\) 从第 \(N\) 项起不小于 \(\varepsilon/2\)，按照严格序的定义，
\[
 |x-a_N|=[\,|a_n-a_N|\,]<\varepsilon.
\]
因此 \(\Q\) 在 \(\widehat{\R}\) 中稠密。这里保留的 \(\varepsilon/2\) 正是严格类不等式所需的正间隙。
