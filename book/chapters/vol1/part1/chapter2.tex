...
交换律直接来自 \(ab=ba\)。以下证明结合律。只需考察 \(q>0\)。若 \(q\in(\alpha\beta)\gamma\)，则存在 \(u\in(\alpha\beta)_+\)、\(c\in\gamma_+\) 使 \(q<uc\)。再由 \(u\in\alpha\beta\) 取 \(a\in\alpha_+,b\in\beta_+\) 使 \(u<ab\)，于是 \(q<abc\)。在 \(q/a\) 与 \(bc\) 之间取有理数 \(v\)，则 \(v\in\beta\gamma\) 且 \(q<av\)，故 \(q\in\alpha(\beta\gamma)\)。反向包含完全对称，非正有理数本来属于两边。
...
最后证明分配律。若正数 \(q\in\alpha(\beta+\gamma)\)，取 \(a\in\alpha_+\) 与 \(v=b+c\in(\beta+\gamma)_+\) 使 \(q<av\)，其中 \(b\in\beta,c\in\gamma\)。不论 \(b,c\) 的符号如何，\(ab\in\alpha\beta\)、\(ac\in\alpha\gamma\)：正的乘积可把 \(a\) 稍向上扰动，非正乘积则按定义属于相应分割。因此 \(ab+ac\in\alpha\beta+\alpha\gamma\)，而后者向下封闭，故 \(q\) 属于右边。

反过来，取 \(q=x+y\)，其中 \(x\in\alpha\beta\)、\(y\in\alpha\gamma\)。可分别选正数 \(a,a'\in\alpha\)、\(b\in\beta\)、\(c\in\gamma\)，使 \(x<ab\)、\(y<a'c\)；当 \(x\le0\) 或 \(y\le0\) 时任取正因子即可。再取 \(d\in\alpha_+\) 且 \(d>\max\{a,a'\}\)，便有
\[
  q=x+y<ab+a'c<d(b+c).
\]
...
\begin{proof}
加法的封闭性、交换律、结合律、零元与逆元已在前文证明，所以 \((\mathbb D,+)\) 是交换群。正分割乘法演算给出正元素上的交换律、结合律和单位元；符号扩张把这些等式逐项传递到任意符号，并且 \(0^*\) 为吸收元。一般分配律已经单独证明。每个正非零分割有正文构造的倒数，负分割 \(\alpha\) 的逆元为 \(-(( -\alpha)^{-1})\)，其乘积按符号定义等于 \(1^*\)。因此 \(\mathbb D\) 是域。

包含关系已经证明为全序。若 \(\alpha\subseteq\beta\)，加法定义直接给出
\(\alpha+\gamma\subseteq\beta+\gamma\)。若 \(0^*\subseteq\alpha,\beta\)，则正部代表的包含给出 \(0^*\subseteq\alpha\beta\)；严格为正时，任取正的 \(a\in\alpha,b\in\beta\)，有理数 \(0<q<ab\) 属于积，故积严格大于 \(0^*\)。特别地，当 \(\alpha\subseteq\beta\) 且 \(0^*\subseteq\gamma\) 时，\(0^*\subseteq\beta-\alpha\)，分配律给出
\[
  \beta\gamma-\alpha\gamma=(\beta-\alpha)\gamma\ge0^*,
\]
即 \(\alpha\gamma\subseteq\beta\gamma\)。所以序与加法及非负数乘法相容，\(\mathbb D\) 是有序域。
\end{proof}
