...
分析中的许多困难并不来自计算，而来自一句话究竟量化了哪些对象、一个集合是否已经给定，以及某个构造是否依赖代表元。以下集中约定全书使用的逻辑、集合与映射语言。正文只在实际需要时说明集合的存在依据；附录A集中解释受限概括、对象编码、商集和可数选择等本卷已经使用的背景。
...
\end{enumerate}
\chapterref{chap:dedekind-construction}和\chapterref{chap:cauchy-construction}将在这一背景中分别构造实数系；确界性质则是构造所得数系的结构定理，不是集合论公理。正文遇到集合构造时只核对当下所需的存在依据。附录A不另开一门集合论课程，只把本卷已经出现的集合构造写清；以后若必须使用 Zorn 引理或更强的选择形式，将在相应章节重新给出准确版本。任何非平凡的选择原则都在首次使用处注明。
...
\[
  \bigl\{\{1\},\{2\}\bigr\}
\]
按包含排序时，两项都是极大元，却没有最大元。\chapterref{chap:real-number-system}中的上界、最大元与上确界也依赖这些区别。
