...
极限定义含有多重且有序的量词。数列 \(a_n\) 收敛到 \(a\) 将在第7章定义为
\[
...
采用三元组定义时，两个映射相等意味着定义域、陪域和图像集合均相同。公式或对应规则只是指定图像集合的一种方式；它还须对定义域中的每个元素给出唯一的陪域元素。
...
\medskip
\noindent\textbf{后文联系。}
第5章以有理 Cauchy 列的等价类构造实数；第86章以几乎处处相等的函数的等价类定义 \(L^p\) 元素。两处都须分别验证运算、次序或范数不依赖代表元。
\medskip
...
命题“每个 \(A_i\) 都可数，则 \(\bigcup_{i\in I}A_i\) 可数”在指标集任意时不成立。例如
\[
  \mathbb R=\bigcup_{x\in\mathbb R}\{x\}
\]
是不可数个单点集的并。可数并定理要求指标集至多可数。
\end{counterexample}
...
\begin{remark}
第3章将在实数系统建立后证明 \(\mathbb R\) 不可数。若采用小数展开证明，必须先排除 \(0.4999\ldots=0.5000\ldots\) 一类表示不唯一所造成的歧义；Cantor 定理本身没有这一技术问题。
\end{remark}
...
\begin{enumerate}[label=\textbf{0.\arabic*.},resume]
  ...
  \item \opendo 从论域、映射的陪域、补集的母集、空交的母集四项中任选三项，分别构造一个因约定不同而真值或对象发生改变的例子。
