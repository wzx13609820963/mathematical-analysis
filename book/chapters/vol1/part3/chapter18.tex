...
\begin{chapterguide}
点连续性只给出每一点附近的局部控制。要在整个区间上获得统一有界、取得极值和介值等全局结论，还需要定义域的结构。闭区间的有限覆盖性质把无限多个局部邻域压缩为有限信息；区间的连通性排除把定义域分成两个彼此分离的非空部分。本章在实数轴上建立这些原型定理。
\end{chapterguide}
...
\begin{counterexample}
实线的覆盖 \(U_n=(-n,n)\)（\(n\in\N\)）没有有限子覆盖，说明无界性也是障碍。
\end{counterexample}
...
反之，若 \(K\) 无界，\(\{(-n,n)\}_{n\in\N}\) 覆盖 \(K\) 而无有限子覆盖。若 \(K\) 不闭，存在聚点 \(a\notin K\)。开集
\[
 U_n=\{x\in\R:\abs{x-a}>1/n\},\qquad n\in\N,
\]
...
两个证明展示了紧致性的两种等价面貌：有限覆盖性质与序列紧致性。在实数轴上，二者都由闭且有界刻画。
...
有界性定理与极值定理可视为连续像保持紧致性的实线推论；介值定理可视为连续像保持连通性的实线推论。这种“结构由连续映射传递”的组织方式将在拓扑卷中推广。

\section*{本章小结}
\addcontentsline{toc}{section}{本章小结}

闭区间的开覆盖可缩减为有限子覆盖，证明依赖开集的局部厚度、闭端点和实数上确界性质。紧致性使连续函数有界并取得极值。实线的连通集恰是区间；连续映射保持连通性，因而连续函数在区间上具有介值性质。紧致性与连通性分别控制“有限化局部信息”和“不中断地填满中间值”。

\section*{习题}
\addcontentsline{toc}{section}{习题}

\noindent\textbf{配置说明：}本章按I级枢纽章配置，共26道编号题，含约44个独立训练单元，建议总用时6至8小时。\exerciserange{18.1}{18.20}为核心必做范围。

\subsection*{A组　覆盖、紧致与极值}
\begin{enumerate}[label=\textbf{18.\arabic*.}]
 \exerciseitem{18.1} \mustdo 分别给出 \((0,1)\)、\([0,\infty)\)、\(\R\) 没有有限子覆盖的具体开覆盖。
 \exerciseitem{18.2} \mustdo 完整重建闭区间有限覆盖定理的上确界证明。
 \exerciseitem{18.3} \mustdo 在上述证明中说明为何 \(\sup E=b\) 之后仍需单独证明 \(b\in E\)。
 \exerciseitem{18.4} \mustdo 证明闭且有界集合 \(K\subset\R\) 具有有限覆盖性质。
 \exerciseitem{18.5} \mustdo 证明具有有限覆盖性质的实数集必有界。
 \exerciseitem{18.6} \mustdo 证明具有有限覆盖性质的实数集必闭。
 \exerciseitem{18.7} \mustdo 分别用有限覆盖法和序列法证明连续函数在闭区间上有界。
 \exerciseitem{18.8} \mustdo 完整证明 Weierstrass 极值定理。
 \exerciseitem{18.9} \mustdo 为“闭”“有界”“连续”三个条件各给一个删去后极值结论失败的反例。
\end{enumerate}

\subsection*{B组　连通与介值}
\begin{enumerate}[label=\textbf{18.\arabic*.},resume]
 \exerciseitem{18.10} \mustdo 证明任意区间连通。
 \exerciseitem{18.11} \mustdo 证明实线中的连通集具有区间性质。
 \exerciseitem{18.12} \mustdo 证明 \(\Q\) 与无理数集都不连通。
 \exerciseitem{18.13} \mustdo 用上确界法证明零点定理，并注明连续性的两次使用。
 \exerciseitem{18.14} \mustdo 从零点定理推出介值定理。
 \exerciseitem{18.15} \mustdo 证明连续函数把区间映为区间。
 \exerciseitem{18.16} \mustdo 证明区间上取值于 \(\Z\) 的连续函数必为常值。
 \exerciseitem{18.17} \mustdo 若连续函数 \(f:[a,b]\to[a,b]\)，证明存在 \(c\in[a,b]\) 使 \(f(c)=c\)。
 \exerciseitem{18.18} \mustdo 证明奇次实系数多项式至少有一个实根。
\end{enumerate}

\subsection*{C组　结构、边界与项目}
\begin{enumerate}[label=\textbf{18.\arabic*.},resume]
 \exerciseitem{18.19} \mustdo 证明连续像保持有限覆盖性质。
 \exerciseitem{18.20} \mustdo 证明连续像保持连通性。
 \exerciseitem{18.21} \optionaldo 证明闭区间中任意无限子集都有聚点，并由此重新证明有限覆盖定理。
 \exerciseitem{18.22} \optionaldo 证明紧致集上的连续双射到实数子集时逆映射连续。
 \exerciseitem{18.23} \projectdo\ 【前置：\chapterref{chap:subsequences-bw}与本章；预计用时：90分钟】建立“闭且有界—序列紧致—有限覆盖性质”的三向证明链，要求每个方向独立注明使用的实数完备性形式。
 \exerciseitem{18.24} \opendo 比较有限覆盖数、Lebesgue 数与一致连续性之间的联系；可预读\chapterref{chap:uniform-continuity}后完成。
 \exerciseitem{18.25} \mustdo 设 \(K_1\supset K_2\supset\cdots\) 为非空紧集。证明若 \(K_1\) 有界，则 \(\bigcap_nK_n\ne\varnothing\)。
 \exerciseitem{18.26} \optionaldo 证明连续函数在紧集上的零点集仍是紧集；若函数处处非零，证明其绝对值有正的最小值。
\end{enumerate}
