...
\begin{proposition}[一点连续延拓]
设 \(a\) 是 \(D\subset\R\) 的聚点且 \(a\notin D\)，并设 \(f:D\to\R\) 在 \(D\) 上连续。函数 \(f\) 能够连续延拓到 \(D\cup\{a\}\)，当且仅当有限极限
\[
 \lim_{\substack{x\to a\\x\in D}}f(x)
\]
存在；此时延拓唯一，并且新点的值只能取为该极限。
\end{proposition}

\begin{proof}
若 \(\widetilde f:D\cup\{a\}\to\R\) 是连续延拓，则连续性给出
\[
 \lim_{\substack{x\to a\\x\in D}}f(x)
 =\lim_{\substack{x\to a\\x\in D}}\widetilde f(x)
 =\widetilde f(a),
\]
所以极限存在且延拓值被唯一确定。反之，若极限为 \(L\)，定义 \(\widetilde f|_D=f\)、\(\widetilde f(a)=L\)。原函数在 \(D\) 上的连续性保证旧点处连续；极限定义保证新点 \(a\) 处连续。因此 \(\widetilde f\) 是所需的唯一连续延拓。
\end{proof}
...
上述分类说明间断以何种方式出现，但有时还需要一个可以估计的量。设 \(a\in D\)。对 \(r>0\) 定义
\[
 \operatorname{osc}_D(f;a,r)
 =\sup\{\abs{f(x)-f(y)}:x,y\in D\cap(a-r,a+r)\}.
\]
若上式中的函数值差无上界，就把振幅记为 \(+\infty\)。随着 \(r\) 减小，参与取上确界的点变少，因此
\(r\mapsto\operatorname{osc}_D(f;a,r)\) 单调不减；相应地，极限
\[
 \operatorname{osc}_D(f;a)
 =\lim_{r\downarrow0}\operatorname{osc}_D(f;a,r)
 =\inf_{r>0}\operatorname{osc}_D(f;a,r)
\]
总在 \([0,+\infty]\) 中存在。这里邻域没有删去中心 \(a\)，所以点值的错误也会被振幅记录下来。

\begin{proposition}[连续性的振幅判据]
函数 \(f:D\to\R\) 在 \(a\in D\) 连续，当且仅当
\[
 \operatorname{osc}_D(f;a)=0.
\]
\end{proposition}

\begin{proof}
若 \(f\) 在 \(a\) 连续，给定 \(\varepsilon>0\)，取 \(\delta>0\) 使
\[
 x\in D,\quad \abs{x-a}<\delta
 \quad\Longrightarrow\quad
 \abs{f(x)-f(a)}<\frac{\varepsilon}{2}.
\]
于是同一邻域内任意 \(x,y\) 满足
\[
 \abs{f(x)-f(y)}
 \le \abs{f(x)-f(a)}+\abs{f(y)-f(a)}<\varepsilon,
\]
故 \(\operatorname{osc}_D(f;a)=0\)。反之，若该振幅为零，给定 \(\varepsilon>0\)，可取 \(r>0\) 使
\(\operatorname{osc}_D(f;a,r)<\varepsilon\)。由于 \(a\in D\cap(a-r,a+r)\)，取 \(y=a\) 便得到
\[
 x\in D,\quad \abs{x-a}<r
 \quad\Longrightarrow\quad
...
对于可去间断点，删去中心点以后邻近函数值的振幅趋于零，而包含中心的振幅为正；对于跳跃间断点，局部振幅至少为
\(\abs{f(a+)-f(a-)}\)。第二类间断中，无界发散使某些局部振幅为无穷，持续振荡则使局部振幅在有限或无限的正值上停留。这些现象说明，间断分类与振幅分别记录了间断的类型和局部函数值的分散程度。
...
可数性证明还给出跳跃大小的定量信息。设递增函数的值都落在有界区间 \([m,M]\) 内。固定 \(\eta>0\)，若内点
\(a_1<\cdots<a_N\) 的跳跃都满足
\[
 f(a_j+)-f(a_j-)\ge\eta,
\]
由单调性，\(f(a_j+)\le f(a_{j+1}-)\)。因此
\[
 N\eta
 \le\sum_{j=1}^{N}\bigl(f(a_j+)-f(a_j-)\bigr)
 \le f(a_N+)-f(a_1-)
 \le M-m.
\]
所以跳跃不小于给定正数的间断点只能有有限多个。把全部间断点写成
\[
 \bigcup_{k=1}^{\infty}
 \{a:f(a+)-f(a-)\ge 1/k\},
\]
又得到至多可数性。这里的有界值域不能随意删除：例如阶梯函数可以在无界区间上有无穷多个大小均为 \(1\) 的跳跃。
...
\begin{enumerate}[label=\textbf{\thechapter.\arabic*.},resume]
 ...
 \exerciseitem{17.16} 给定可数无限集 \(A\subset(0,1)\)，构造一个在 \(A\) 的每一点发生跳跃的有界递增函数。
...
\begin{enumerate}[label=\textbf{\thechapter.\arabic*.},resume]
 ...
 \optionalexerciseitem{17.21} 证明严格单调函数相对于其像集总有连续逆，并构造一个不连续的严格单调函数，说明其像集可以不是区间。
 \optionalexerciseitem{17.22} 构造处处不连续但在每一点左右极限都存在的函数是否可能？给出证明。
 \projectexerciseitem{17.23} \ 【前置：\chapterref{chap:mathematical-language}可数性与\chapterref{chap:real-number-system}有理数稠密性；预计用时：75分钟】重建单调函数间断点至多可数的证明，并进一步证明跳跃大于 \(1/n\) 的间断点在有界值域内只有有限多个。
 ...
 \optionalexerciseitem{17.26} 设 \(f:D\to\R\)。证明：若对每个有理数 \(p<q\)，集合 \(f^{-1}((p,q))\) 都在 \(D\) 中相对开，则 \(f\) 连续。
\end{enumerate}
