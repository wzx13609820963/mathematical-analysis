...
\begin{definition}[紧集]
若集合 \(K\subset\R\) 的每个开覆盖都有有限子覆盖，则称 \(K\) 为\term{紧集}。这一名称只缩写有限覆盖性质，不预先包含“闭且有界”；两者在实线上等价，是下面要证明的 Heine--Borel 定理。
\end{definition}

定义中的覆盖成员取为实线中的开集。若改用 \(K\) 中的相对开集，定义不变。事实上，任一相对开集都可写成 \(K\cap U\)，其中 \(U\subset\R\) 开；相对开覆盖
\(\{K\cap U_\lambda\}\) 有有限子覆盖，当且仅当相应的 \(\{U_\lambda\}\) 从 \(K\) 上看有有限子覆盖。这个简单的转换使连续映射的原像可以直接用于紧致性论证。

\begin{proposition}[闭集的序列刻画]
集合 \(F\subset\R\) 闭，当且仅当每个满足 \(x_n\in F\)、\(x_n\to x\) 的数列都有 \(x\in F\)。
\end{proposition}

\begin{proof}
若 \(F\) 闭而 \(x\notin F\)，则开集 \(\R\setminus F\) 含有 \(x\) 的某个邻域；收敛到 \(x\) 的数列最终落入这个邻域，不可能始终属于 \(F\)。

反之，若 \(F\) 不闭，则 \(\R\setminus F\) 不是开集。存在 \(x\notin F\)，使 \(x\) 的每个邻域都与 \(F\) 相交。对每个 \(n\in\Npos\) 取
\[
 x_n\in F\cap(x-1/n,x+1/n).
\]
于是 \(x_n\to x\notin F\)，序列条件失败。
\end{proof}
...
设 \(a\le b\)。闭区间 \([a,b]\) 的每个开覆盖都有有限子覆盖。
\end{theorem}
...
集合 \(K\subset\R\) 紧，当且仅当 \(K\) 闭且有界。
\end{theorem}

\begin{proof}
若 \(K\) 闭且有界，则 \(K\subset[-M,M]\)。在 \(K\) 的任一开覆盖中加入开集 \(\R\setminus K\)，得到 \([-M,M]\) 的开覆盖；闭区间定理给出有限子覆盖，删去可能出现的 \(\R\setminus K\) 后，余下成员覆盖 \(K\)。

反之，若 \(K\) 无界，\(\{(-n,n)\}_{n\in\Npos}\) 覆盖 \(K\) 而无有限子覆盖。若 \(K\) 不闭，由闭集的序列刻画，可取 \(x_j\in K\) 使 \(x_j\to a\notin K\)。开集
\[
 U_n=\{x\in\R:\abs{x-a}>1/n\},\qquad n\in\Npos,
\]
覆盖 \(K\)，但任意有限子族的并仍避开 \(a\) 的某个邻域；数列 \((x_j)\) 最终进入该邻域，故这个有限子族不能覆盖 \(K\)。
\end{proof}

\begin{proposition}[紧集的基本运算]
空集与有限集紧；紧集的闭子集紧；有限多个紧集的并仍紧。
\end{proposition}

\begin{proof}
空集没有需要覆盖的点，有限集只需为每个点选一个覆盖成员。设 \(F\subset K\)，其中 \(F\) 在 \(K\) 中相对闭且 \(K\) 紧。给定 \(F\) 的相对开覆盖，把 \(K\setminus F\) 添入便得到 \(K\) 的相对开覆盖；取有限子覆盖后删去 \(K\setminus F\)，余下成员覆盖 \(F\)。最后，若 \(K_1,\ldots,K_m\) 紧，则给定它们的并集的开覆盖，分别从每个 \(K_j\) 上取有限子覆盖，把这有限多组成员合在一起即可。
\end{proof}

\begin{theorem}[Lebesgue 数引理]
设 \(K\subset\R\) 紧，\(\mathcal U\) 是 \(K\) 的开覆盖。则存在 \(\lambda>0\)，使对每个 \(x\in K\)，都能找到 \(U\in\mathcal U\) 满足
\[
 K\cap(x-\lambda,x+\lambda)\subset U.
\]
这样的 \(\lambda\) 称为该覆盖的一个 \term{Lebesgue 数}。
\end{theorem}

\begin{proof}
若 \(K=\varnothing\)，任意 \(\lambda>0\) 都满足结论。以下设 \(K\ne\varnothing\)。
对每个 \(x\in K\)，选取 \(U_x\in\mathcal U\) 使 \(x\in U_x\)。由开性可取 \(r_x>0\)，使
\[
 (x-2r_x,x+2r_x)\subset U_x.
\]
较小的区间 \(\{(x-r_x,x+r_x):x\in K\}\) 覆盖 \(K\)。由紧致性选出有限多个中心
\(x_1,\ldots,x_m\)，并令
\[
 \lambda=\min_{1\le j\le m}r_{x_j}>0.
\]
任取 \(y\in K\)，某个 \(j\) 满足 \(\abs{y-x_j}<r_{x_j}\)。若 \(z\in K\) 且 \(\abs{z-y}<\lambda\)，则
\[
 \abs{z-x_j}\le\abs{z-y}+\abs{y-x_j}<2r_{x_j},
\]
所以 \(z\in U_{x_j}\)。因此 \(K\cap(y-\lambda,y+\lambda)\subset U_{x_j}\)。
\end{proof}

\begin{corollary}[实线上的序列紧致性]
集合 \(K\subset\R\) 紧，当且仅当 \(K\) 中每个数列都有一个收敛到 \(K\) 中某点的子列。
\end{corollary}

\begin{proof}
若 \(K\) 紧，则它闭且有界。任取 \((x_n)\subset K\)，Bolzano--Weierstrass 定理给出收敛子列；闭集的序列刻画保证其极限仍在 \(K\) 中。

反之，若 \(K\) 无界，可选 \(x_n\in K\) 使 \(\abs{x_n}>n\)，该列没有收敛子列。若 \(K\) 不闭，取 \(x_n\in K\) 收敛到 \(a\notin K\)；它的每个子列仍收敛到 \(a\)，不可能有子列收敛到 \(K\) 中的点。故序列条件推出 \(K\) 闭且有界，再由 Heine--Borel 刻画知 \(K\) 紧。
\end{proof}

\section{连续函数的有界性和最值定理}

\subsection{有界性定理}

\begin{theorem}[有界性定理]
若 \(K\subset\R\) 紧且 \(f:K\to\R\) 连续，则 \(f\) 在 \(K\) 上有界。
\end{theorem}

\begin{proof}[覆盖证明]
空集上的函数有界，以下设 \(K\ne\varnothing\)。
对每个 \(x\in K\)，由连续性取开邻域 \(U_x\)，使
\[
 y\in U_x\cap K\Longrightarrow\abs{f(y)-f(x)}<1.
\]
这些 \(U_x\) 构成 \(K\) 的开覆盖。取有限子覆盖
\[
 U_{x_1},\ldots,U_{x_m}.
\]
令
...
\begin{proof}[序列证明]
若 \(f\) 无界，可对每个 \(n\) 选 \(x_n\in K\) 使 \(\abs{f(x_n)}>n\)。由序列紧致性，\((x_n)\) 有收敛子列 \(x_{n_k}\to x\in K\)。连续性给出
\[
 f(x_{n_k})\to f(x),
\]
...
第一个证明从有限子覆盖取得有限组局部界，第二个证明从点列抽取收敛子列。在实数轴上，这两种论证分别对应紧致性的覆盖形式和序列形式。闭区间只是最常用的紧集；任意闭有界子集都同样适用。

\subsection{最大值最小值定理}

有界性只产生数 \(M=\sup f(K)\)，并不保证存在 \(x\) 使 \(f(x)=M\)。取得最大值还需要两步：从上确界附近选出一列函数值，再从相应点列抽取仍留在 \(K\) 中的收敛子列。连续性最后把点列极限送到函数值极限。

\begin{theorem}[Weierstrass 极值定理]
若 \(K\subset\R\) 是非空紧集，\(f:K\to\R\) 连续，则存在 \(x_{\min},x_{\max}\in K\)，使
\[
...
\[
 S=f(K)
\]
有上界。令 \(M=\sup S\)。按上确界定义，对每个 \(n\) 选 \(x_n\in K\) 使
\[
 M-\frac1n<f(x_n)\le M.
\]
由序列紧致性，某子列 \(x_{n_k}\to x_{\max}\in K\)。连续性与夹逼给出
\[
 f(x_{\max})
...
\begin{corollary}[到紧集的距离]
设 \(K\subset\R\) 为非空紧集，\(x\in\R\)。则存在 \(y_x\in K\) 使
\[
 \operatorname{dist}(x,K)
 :=\inf_{y\in K}\abs{x-y}
 =\abs{x-y_x}.
\]
若非空紧集 \(K,L\subset\R\) 不相交，则
\[
 \operatorname{dist}(K,L)
 :=\inf\{\abs{x-y}:x\in K,\ y\in L\}>0.
\]
\end{corollary}

\begin{proof}
第一个结论对连续函数 \(y\mapsto\abs{x-y}\) 应用极值定理即可。对于第二个结论，若下确界为零，可选
\(x_n\in K\)、\(y_n\in L\) 使 \(\abs{x_n-y_n}<1/n\)。从 \((x_n)\) 中抽出收敛子列 \(x_{n_k}\to x\in K\)，再从相应的 \((y_{n_k})\) 中抽出子列 \(y_{n_{k_j}}\to y\in L\)。由于
\[
 \abs{x_{n_{k_j}}-y_{n_{k_j}}}\longrightarrow0,
\]
唯一性给出 \(x=y\)，与 \(K\cap L=\varnothing\) 矛盾。
\end{proof}
...
\section{连续映射保持紧致性和连通性}

\begin{theorem}[连续像保持有限覆盖性质]
设 \(K\subset\R\) 紧，\(f:K\to\R\) 连续。则 \(f(K)\) 也紧。
\end{theorem}
...
\[
 f^{-1}(V_\lambda)
\]
在 \(K\) 中相对开，并且这些原像覆盖 \(K\)。把它们写成 \(K\cap U_\lambda\)（\(U_\lambda\) 为实线开集），则 \(\{U_\lambda\}\) 是 \(K\) 的开覆盖。由紧致性选出有限多个，对应的 \(V_\lambda\) 便覆盖 \(f(K)\)。
\end{proof}

\begin{corollary}[紧集上的连续双射]
设 \(K\subset\R\) 紧，\(Y\subset\R\)，并设 \(f:K\to Y\) 是连续双射。则逆映射
\[
 f^{-1}:Y\to K
\]
连续。
...
\begin{proof}
若 \(C\subset K\) 相对闭，则 \(C\) 是紧集，因而 \(f(C)\) 也是紧集。实线上的紧集闭，所以 \(f(C)\) 在 \(Y\) 中相对闭。现在
\[
 (f^{-1})^{-1}(C)=f(C),
\]
故逆映射满足闭集原像判据。这里不能只写“连续双射的逆连续”：紧致性正是把闭集的像仍变成闭集的条件。
\end{proof}
...
闭区间上连续函数的整体性质同时使用了定义域和映射本身的条件。下面把前文分散出现的失败情形集中比较。

若定义域不是闭集，连续函数即使有界也未必取得端点值。例如 \(f(x)=x\) 在 \((0,1)\) 上既无最大值也无最小值。若定义域闭但无界，\(f(x)=x\) 在 \([0,\infty)\) 上无界。若定义域闭且有界但函数不连续，令
\[
 f(0)=0,\qquad f(x)=\frac1x\quad(0<x\le1),
\]
则 \(f\) 在 \([0,1]\) 上无界。这三例分别说明不能只保留闭、有界、连续中的两项。

介值定理中的区间条件也不可删除。集合
\[
 E=[-1,0]\cup[1,2]
\]
不是连通集，恒等映射 \(f:E\to\R\) 连续，却取到 \(0\) 和 \(1\) 而不取它们之间的任何数。失败来自定义域本身的缺口，不是连续性的局部条件失效。

最后，紧致性在实线上等价于闭且有界，但这种表述不能提前当作一般空间中的定义。第 34 章以后，紧致仍由有限开覆盖定义；“闭且有界”是否足够则要依据空间另行判断。
...
\begin{enumerate}[label=\textbf{\thechapter.\arabic*.},resume]
 ...
 \exerciseitem{18.24}  设 \(\mathcal U\) 是紧集 \(K\) 的开覆盖。若 \(\lambda>0\) 满足：每个 \(x\in K\) 的相对邻域 \(K\cap(x-\lambda,x+\lambda)\) 都包含于某个 \(U\in\mathcal U\)，则称 \(\lambda\) 为覆盖的 Lebesgue 数。完整证明每个紧集开覆盖都有 Lebesgue 数。
