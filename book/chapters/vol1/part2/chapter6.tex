...
压缩估计同时控制存在性、唯一性和速度。它把相邻两次迭代的误差按固定比例缩小，有限几何尾和于是控制任意两项的累积差。这里要求 \(q<1\) 是定量条件；若只有 \(q=1\)，映射 \(f(x)=1-x\) 已表明迭代可以永久振荡。\chapterref{chap:equivalent-completeness}已经建立实数的 Cauchy 完备性，下面用它把这种内部误差控制转化为极限；\chapterref{chap:cauchy-criterion}将系统整理 Cauchy 判据。
...
\begin{enumerate}[label=\textbf{\thechapter.\arabic*.},resume]
 ...
 \projectexerciseitem{10.20} \ 【前置：\chapterref{chap:equivalent-completeness}Cauchy完备性；预计用时：75分钟】证明闭区间上的压缩迭代定理，不使用单调有界收敛定理；再说明闭区间条件在何处使用。
