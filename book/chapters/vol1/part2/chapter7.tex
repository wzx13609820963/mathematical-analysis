...
\]
其中闭包符号在本章只表示“集合连同其全部聚点”；\chapterref{chap:continuous-functions-closed-interval}将给出闭集语言。
...
\begin{enumerate}[label=\textbf{\thechapter.\arabic*.},resume]
 ...
 \projectexerciseitem{11.18} \ 【前置：\chapterref{chap:equivalent-completeness}区间套；预计用时：75分钟】完整重建 Bolzano--Weierstrass 定理的二分区间证明，并给出子列误差不超过 \(2^{-k}(B-A)\) 的定量版本。
